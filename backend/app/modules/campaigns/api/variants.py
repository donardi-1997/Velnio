...
async def list_variants(
    ...
    await _get_campaign(db, campaign_id, workspace.id)
    result = await db.execute(
...
async def create_variant(
    ...
    if len(active_result.scalars().all()) >= 4:
        raise BadRequestException("Maximum 4 active variants allowed")

    existing_result = await db.execute(select(LandingVariant).where(LandingVariant.campaign_id == campaign_id))
    ...
    if data.clone_from_variant_id:
        source_result = await db.execute(select(LandingVariant).where(LandingVariant.id == data.clone_from_variant_id))
        ...
        if source_variant.landing_page_id:
            source_lp_result = await db.execute(select(LandingPage).where(LandingPage.id == source_variant.landing_page_id))
...
async def update_traffic_weights(
    ...
    result = await db.execute(select(LandingVariant).where(LandingVariant.campaign_id == campaign_id))
...
async def update_variant(
    ...
    await _get_campaign(db, campaign_id, workspace.id)
    result = await db.execute(select(LandingVariant).where(
