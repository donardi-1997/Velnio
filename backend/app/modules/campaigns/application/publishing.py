...
class CampaignPublishingService:
    ...
    async def readiness(self, campaign_id: UUID, workspace_id: UUID) -> dict:
        ...
        offer = (await self.db.execute(select(Offer).where(Offer.campaign_id == campaign_id))).scalar_one_or_none()
        landing = (await self.db.execute(select(LandingPage).where(LandingPage.campaign_id == campaign_id))).scalar_one_or_none()

        images = []
        if campaign.product_id and product is not None:
            images = (
                await self.db.execute(select(ProductImage).where(ProductImage.product_id == campaign.product_id))
            ).scalars().all()
        ...
        add_check("store_connected", store_connected, "Store connected" if store_connected else "No connected store")
        ...
        add_check("has_visual_direction", has_visual_direction, "Visual direction set" if has_visual_direction else "No visual direction")
    ...
    async def publish(self, campaign_id: UUID, workspace_id: UUID) -> dict:
        ...
        if campaign.status == CampaignStatus.PUBLISHED and campaign.external_product_id:
            return {"status": "published", "provider": "existing", "shopify_product_id": campaign.external_product_id, "shopify_page_id": campaign.external_page_id}
        ...
        try:
            publish_result = await get_shopify_provider().publish_campaign(campaign, product, store, angle, landing, offer)
            ...
            await self.db.execute(
                sa_update(Campaign).where(Campaign.id == campaign_id, Campaign.workspace_id == workspace_id).values(
            ...
            await self.db.flush()
            return {"status": "published", "provider": publish_result.get("provider", "mock"), "shopify_product_id": str(shopify_product_id), "shopify_page_id": publish_result.get("shopify_page_id")}
